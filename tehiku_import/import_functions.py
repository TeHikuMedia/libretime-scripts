...
def convert_media(from_format, to_format, file_path):
    cmd = 'ffmpeg -y -i "%s" -c:a libvorbis -q:a 5 "%s"' % (
        file_path, file_path.replace(from_format, to_format))
    Popen(cmd.split(' '))
    Popen(['rm', file_path])
...
def scale_media(
        file_path, target_length, length=0, max_scale=0.05, max_seconds_delta=1,
        extension='mp3'
):
    if length == 0:
        command = \
            'ffprobe -i %s -show_entries format=duration -v quiet -of json' % (
                file_path)
    ...
    if abs(target_length - length) < max_seconds_delta:
        print("Not Scaling as Duration is withing %d seconds." %
              (max_seconds_delta))
        scale = 1
        ...
        if scale > (1+max_scale):
            scale = 1+max_scale
        elif scale < (1-max_scale):
            scale = 1-max_scale
        print("Length = %d:%02d.%d" %
              (media_length['mins'], media_length['secs'], media_length['hunds']))
        ...
        with tempfile.NamedTemporaryFile(suffix=f".{extension}", delete=False) as tmp:
            ...
            print(out)
            print(file_path)
            cmd = 'ffmpeg -y -i %s -af atempo=%0.04f -vn %s' % (
                file_path, scale, out)
...
def convert_audio(file):
    ...
    command = '/usr/bin/ffmpeg -y -i {0} -b:a 128k {1}'.format(
        file, new_file_name)
    p = Popen(command.split(' '), stdout=PIPE, stderr=PIPE)
