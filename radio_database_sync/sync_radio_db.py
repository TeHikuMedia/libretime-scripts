...
parser.add_argument("-w", "--watch", help="Watch the folder.", action="store_true")
parser.add_argument("-s", "--sync", help="Sync the folder", action="store_true")
...
def get_key(md5, data):
    ...
    """
    return md5
    return "-".join([md5] + [str(data[i]) for i in LABEL_KEYS if data[i]])
...
def load_radio_db(keep_duplicates=False):
    ...
    response = requests.get(API_URL, auth=LIBRETIME_BASIC_AUTH)
    files = response.json()
    data = {}
    ...
    for file in files:
        ...
        if file["md5"] in data.keys():
            ...
            if any(
                [file[key].find("#") > -1 for key in LABEL_KEYS[:3] if file[key]]
                + [any([file[key] == "" for key in LABEL_KEYS[:3] if file[key]])]
            ):
                print(
            ...
            elif all([file[key] == data[file["md5"]][key] for key in LABEL_KEYS]):
                print(
...
def delete_file(file_id, SESSION_ID):
    ...
    API_URL = f"{LIBRETIME_URL}/api/v2/files/{file_id}"
    response = requests.delete(API_URL, auth=LIBRETIME_BASIC_AUTH)
    try:
        response.raise_for_status()
    except Exception:
        return response.status_code
...
def sync_entire_folder(delete=args.delete, update_metadata=False):
    ...
    for md5 in libretime_db.keys():
        if any(
            [
                ...
                for key in LABEL_KEYS[:3]
                if libretime_db[md5][key]
            ]
        ):
            print(
                "Delete stale file",
    ...
    for md5 in exists:
        ...
        if any(libretime_db[md5][i] != db[md5][i] for i in LABEL_KEYS):
            print("Data outdated")
            payload = {
                **{key: db[md5][key] for key in LABEL_KEYS},
                **{key: libretime_db[md5][key] for key in REQUIRED},
            ...
            print(payload)
            update_file(libretime_db[md5]["id"], payload)

        # Delete files with # in our tags
        elif any(
            [db[md5][key].find("#") > -1 for key in LABEL_KEYS[:3] if db[md5][key]]
        ):
            print("Delete file, it has a # in its name", libretime_db[md5]["name"])
            delete_file(libretime_db[md5]["id"], session_id)
    ...
    for md5 in new:
        ...
        if any([db[md5][key].find("#") > -1 for key in LABEL_KEYS[:3] if db[md5][key]]):
            continue
        else:
    ...
    for _, file in db.items():
        ...
        if file["md5"] in _keys.keys():
            ...
            if file["md5"] not in db.keys():
                data[file["md5"]] = [db[_keys[file["md5"]]], file]
            else:
...
def process_path(path, root_folder):
    ...
    try:
        ...
        language = None
        logging.warning(
            "File not in language folder: {0}".format(os.path.join(RELATIVE, name))
        )
    ...
    try:
        ...
        genre = None
        logging.debug(
            "File not in genre folder: {0}".format(os.path.join(RELATIVE, name))
        )
    ...
    if not audio:
        ...
        if os.path.exists(file_path):
            ...
            try:
                ...
                logging.warning(
                    "Could not load file with mutagen after conversion: {0}".format(
                        name
                    )
                )
                return data
            ...
            if [language] != lang:
                ...
                try:
                    ...
                    try:
                        ...
                        logging.warning(
                            "Could now write 'language' to {0}".format(name)
                        )
                        return data
                SAVE = True
...
class MyRegexMatchingEventHandler(RegexMatchingEventHandler):
    ...
    def _match_file(self, data):
        for key in self.db:
            ...
            if all(
                [
                    (
                        data["fullname"] in item["filepath"]
                        or data["name"] in item["name"]
                    ),
    ...
    def process_file(self, new_data, deleted=False, created=False, updated=False):
        status = 200
        md5 = new_data["md5"]
        ...
        if deleted and original:
            ...
            status = delete_file(original["id"], self.session_id)
            if status > 399:
            ...
            if md5_changed:
                ...
                status = upload_file(original["path"])
                if status == 201:
                    # Now update the file
                    sleep(5)
                    self.refresh_db()
            ...
            payload["name"] = (
                original["name"] if original["name"] != "" else new_data["name"]
            )
            payload["accessed"] = 0
            payload["updated_at"] = datetime.datetime.now(
                tz=pytz.timezone("Pacific/Auckland")
            ...
            status = update_file(original["id"], payload)
